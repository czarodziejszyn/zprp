...
def normalize(obj):
    if isinstance(obj, dict):  # from cache
        return (obj.get("objtype"), obj.get("latitude"), obj.get("longitude"))

    else:  # from api
        return (obj.objtype, obj.latitude, obj.longitude)
...
async def main():
    ...
    # Police stations
    police_count = await import_with_cache(
        fetch_police_stations, "police_station", "police stations"
    )
    ...
    # Stops
    stops_count = await import_with_cache(fetch_stops, "stop", "stops")
    ...
    # Accomodiations
    accommodations_count = await import_with_cache(
        fetch_accommodations, "accommodation", "accommodations"
    )
